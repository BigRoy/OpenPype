...
"""
C4D handles all imports and exports of file types through import and export 
plugins. There is a special convoluted way to get the settings for the plugins
by supplying the plugin ID and getting a dict with the settings that can be 
set.

By default exporters sticky the previous settings, so for consistency and 
reliability set each possible setting each time. Use the exporter_settings 
context manager to return the settings to their previous state.
"""
...
def exporter_settings(exporter_id, defaults={}):
    ...
    if exporter:
        ...
        for key, value in defaults.items():
            previous_settings[key] = {"id": value["id"],
                                      "default": exporter[value["id"]]}
    else:
...
def export_abc(filepath, settings={}, doc=None):
    ...
    with exporter_settings(c4d.FORMAT_ABCEXPORT, defaults) as exporter:
        ...
        c4d.documents.SaveDocument(doc, filepath,
